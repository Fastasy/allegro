...
export const SplashSection: React.FC = () => {
  return (
    <section className="relative h-screen bg-black flex flex-col items-center justify-center overflow-hidden border-b border-zinc-900">
      {/* Background glow */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-96 h-96 bg-yellow-400/10 rounded-full blur-[140px] pointer-events-none animate-pulse-glow" />
      ...
      <div className="absolute bottom-10 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 text-zinc-500 animate-fade-scale" style={{ animationDelay: '800ms' }}>
        <span className="text-[10px] font-bold uppercase tracking-[0.2em]">Scroll Down</span>
        <ChevronDown className="w-4 h-4 animate-bounce-down text-yellow-400/70" />
      </div>
    </section>
  );
};
